  ...
  <main class="container px-4 py-8 mx-auto text-white md:px-8 lg:px-16">
    ...
    <template v-else-if="aboutContent">
        <section class="min-h-[20rem] h-[30rem] grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            <div
                v-if="aboutContent.image_1_url"
                class="w-full hidden md:block bg-center bg-cover rounded-2xl brand-border card-home"
                :style="{ backgroundImage: `url(/storage/${aboutContent.image_1_url})` }"
            ></div>
            <div
                v-if="aboutContent.image_2_url"
                class="w-full bg-top bg-cover rounded-2xl brand-border card-home"
                :style="{ backgroundImage: `url(/storage/${aboutContent.image_2_url})` }"
            ></div>
        </section>

        <section class="mb-8 p-4 md:p-0">
        ...
        <section class="flex flex-col md:flex-row gap-8 mb-8 items-stretch">
            ...
            <div class="card w-full md:w-1/3 flex flex-col">
                ...
                <div class="flex flex-col space-y-4 h-full justify-center">
                    <div v-for="(lang, index) in aboutContent.languages" :key="index" class="flex items-center text-brand-text">
                        <img v-if="lang.icon_url" :src="`/storage/${lang.icon_url}`" :alt="lang.name" class="w-14  h-14  object-cover rounded-full mr-3 flex-shrink-0 bg-gray-700 p-1"/>
        ...
        <EducationTimeline :educations="educations" class="bg-brand-dark" />
    </template>
  </main>
</template>
